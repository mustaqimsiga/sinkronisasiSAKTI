merge INTO sakti_ut.ADM_R_BKPK arb USING
...
FROM
  (SELECT substr(ffv.flex_value,1,4) flex_value,
    CASE
  ...
  AND substr(ffv.flex_value,5,2) LIKE '%00'
  AND substr(ffv.flex_value,4,1) != '0'
  AND (ffv.flex_value NOT LIKE 'B%' AND ffv.flex_value NOT LIKE 'C%' AND ffv.flex_value NOT LIKE 'T%')
  ) ffv
FULL OUTER JOIN sakti_ut.ADM_R_BKPK arb
...
WHEN matched THEN
  ...
  INSERT
    ...
    (
      ...
      0,
      '-', -- not found
      substr(src.flex_value,1,3)
    ); 
    commit;
    
   select * from sakti_ut.ADM_R_BKPK ; 
    SELECT distinct substr(ffv.flex_value,1,3) flex_value
  FROM fnd_flex_values@SPAN_ST ffv
  LEFT JOIN fnd_flex_value_sets@SPAN_ST ffvs
  ON ffv.flex_value_set_id = ffvs.flex_value_set_id
  LEFT JOIN fnd_flex_values_tl@SPAN_ST ffvt
  ...
  AND substr(ffv.flex_value,4,1) != '0'
  AND (ffv.flex_value NOT LIKE 'B%' AND ffv.flex_value NOT LIKE 'C%' AND ffv.flex_value NOT LIKE 'T%')
  and substr(ffv.flex_value,1,3) not in (  
  select distinct kode from sakti_ut.ADM_R_KBKPK) 
  ;